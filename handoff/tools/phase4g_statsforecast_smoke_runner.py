...
ENV_NAME = "environments/statsforecast-py313"
RUNTIME = ROOT / ENV_NAME / ".venv/bin/python"
ENV_PROJECT = SOURCE_WT / ENV_NAME
RUN_ID = datetime.now().astimezone().strftime("%Y%m%d-%H%M%S")
LOCAL_OUT = ROOT / "artifacts" / f"phase4g-statsforecast-smoke-{RUN_ID}"
HANDOFF_OUT = HANDOFF / "phase4g"
MODEL_DIR = LOCAL_OUT / "models"
CHILD = LOCAL_OUT / "statsforecast-child.py"

EXPECTED_STATSFORECAST = "2.1.1"
EXPECTED_PYTHON_PREFIX = "3.13."
MODEL_PUBLIC_NAME = "Naive"
BASELINES = (
    "random",
    "fixed",
    "mean",
...
def prerequisite_gate() -> dict[str, str]:
    phase4f_path = HANDOFF / "phase4f/summary.json"
    if not phase4f_path.exists():
        raise RuntimeError("PHASE4F_SUMMARY_MISSING")
    phase4f = json.loads(phase4f_path.read_text("utf-8"))
    if phase4f.get("status") != "VERIFIED":
        raise RuntimeError("PHASE4F_NOT_VERIFIED")
    ...
    if row is None:
        raise RuntimeError("STATSFORECAST_NOT_IN_PHASE4_READY_QUEUE")
    if row.get("phase4_smoke_allowed") != "True":
        raise RuntimeError("STATSFORECAST_PHASE4_SMOKE_NOT_ALLOWED")
    if row.get("lane") != "CURRENT_CPU_LEGACY":
        raise RuntimeError(f"STATSFORECAST_UNEXPECTED_LANE:{row.get('lane')}")
    return row


def runtime_probe() -> dict[str, Any]:
    if not RUNTIME.exists() or not os.access(RUNTIME, os.X_OK):
        raise RuntimeError(f"STATSFORECAST_RUNTIME_MISSING:{RUNTIME}")
    code = r'''
...
payload = {
    ...
    "naive_module": Naive.__module__,
    "naive_class": Naive.__name__,
}
...
'''
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"STATSFORECAST_RUNTIME_PROBE_FAILED:{proc.stderr.strip()}")
    payload = json.loads(proc.stdout.strip().splitlines()[-1])
    if payload.get("statsforecast") != EXPECTED_STATSFORECAST:
        raise RuntimeError(f"STATSFORECAST_VERSION_MISMATCH:{payload.get('statsforecast')}")
    if not str(payload.get("python", "")).startswith(EXPECTED_PYTHON_PREFIX):
        raise RuntimeError(f"STATSFORECAST_PYTHON_MISMATCH:{payload.get('python')}")
    if not str(payload.get("prefix", "")).endswith(f"/{ENV_NAME}/.venv"):
        raise RuntimeError(f"STATSFORECAST_PREFIX_MISMATCH:{payload.get('prefix')}")
    if payload.get("naive_class") != MODEL_PUBLIC_NAME:
        raise RuntimeError("STATSFORECAST_NAIVE_IMPORT_MISMATCH")
    if payload.get("numpy") is None or payload.get("pandas") is None:
        raise RuntimeError("STATSFORECAST_REQUIRED_TRANSITIVE_RUNTIME_MISSING")
    return payload
...
def environment_contract() -> dict[str, Any]:
    ...
    if not pyproject.exists():
        raise RuntimeError(f"STATSFORECAST_PYPROJECT_MISSING:{pyproject}")
    ...
    if not all(token in text for token in required):
        raise RuntimeError("STATSFORECAST_PYPROJECT_CONTRACT_MISMATCH")
...
def prepare(source: Path, output: Path, meta_path: Path, expected_positions: list[str], draw_col: str):
    ...
    dump(meta_path, {
        ...
        "statsforecast": importlib.metadata.version("statsforecast"),
    })
...
def fit(data_path: Path, model_dir: Path, result_path: Path, positions: list[str], min_value: int, max_value: int):
    ...
    for index, col in enumerate(positions, start=1):
        ...
        model = Naive()
        fitted = model.fit(y)
        pred_dict = fitted.predict(h=1)
        pred = np.asarray(pred_dict["mean"], dtype=float).reshape(-1)
        ...
        models.append({
            ...
            "alias": getattr(fitted, "alias", None),
...
def gpu_process_rows() -> tuple[bool, list[dict[str, Any]]]:
    proc = run(
        [
            "nvidia-smi",
...
def monitored_child(
    ...
    *,
    ...
    with stdout_path.open("w", encoding="utf-8") as out, stderr_path.open("w", encoding="utf-8") as err:
        ...
        while proc.poll() is None:
            ...
            if elapsed > timeout:
                ...
                raise RuntimeError(f"STATSFORECAST_CHILD_TIMEOUT:{args[0]}:{timeout}")
            pids = descendants(proc.pid)
            monitor_ok, gpu_rows = gpu_process_rows()
            if not monitor_ok:
                nvidia_smi_failures += 1
    ...
    return {
        ...
        "nvidia_smi_failures": nvidia_smi_failures,
        "samples": samples,
    }
...
def prepare_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if proc.returncode != 0 or not meta.exists():
        raise RuntimeError(f"STATSFORECAST_DATA_PREPARE_FAILED:{proc.stderr.strip()}")
    ...
    for key in identity_keys:
        ...
        if expected != actual:
            raise RuntimeError(f"STATSFORECAST_DATA_IDENTITY_MISMATCH:{key}:expected={expected}:actual={actual}")
    if observed.get("cuda_visible_devices") != "":
        raise RuntimeError("STATSFORECAST_DATA_PREPARE_CPU_ENV_NOT_PINNED")
    return observed
...
def execute_lifecycle(data: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if fit_monitor["returncode"] != 0 or not fit_result.exists():
        ...
        raise RuntimeError(f"STATSFORECAST_FIT_PROCESS_FAILED:rc={fit_monitor['returncode']}:{stderr[-2000:]}")
    ...
    if reload_monitor["returncode"] != 0 or not load_result.exists():
        ...
        raise RuntimeError(f"STATSFORECAST_RELOAD_PROCESS_FAILED:rc={reload_monitor['returncode']}:{stderr[-2000:]}")
...
def validate_lifecycle(
    ...
    checks = {
        ...
        "statsforecast_version_fit": fit.get("statsforecast") == EXPECTED_STATSFORECAST,
        "statsforecast_version_reload": reload.get("statsforecast") == EXPECTED_STATSFORECAST,
        ...
        "gpu_monitor_fit_healthy": monitors["fit"].get("nvidia_smi_failures") == 0,
        "gpu_monitor_reload_healthy": monitors["reload"].get("nvidia_smi_failures") == 0,
...
def publish(summary: dict[str, Any]) -> str:
    if summary.get("status") != "VERIFIED":
        raise RuntimeError("REFUSE_TO_PUBLISH_NON_VERIFIED_PHASE4G")
    ...
    for src in sorted(LOCAL_OUT.rglob("*")):
        ...
        if "models" in rel.parts or rel.name == "smoke-input.csv" or rel == Path("statsforecast-child.py"):
            continue
    ...
    report = HANDOFF_OUT / "PHASE4G_REPORT.md"
    report.write_text(
        "\n".join(
            [
    ...
    handoff.setdefault("completed_phases", {})["phase4g"] = "VERIFIED"
    handoff["current_phase"] = "phase4g_statsforecast_verified_phase4h_next"
    handoff["phase4g"] = summary
    ...
    if staged.returncode == 1:
        commit = run(
            [
                ...
                "commit",
                "-m",
                f"audit: publish Phase 4G StatsForecast smoke {RUN_ID}",
            ],
            timeout=120,
        )
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "schema_version": 1,
        "phase": "PHASE4G_STATSFORECAST_PY313_CPU_LIFECYCLE",
        "run_id": RUN_ID,
        "status": "FAILED",
        "source_sha": EXPECTED_SOURCE_SHA,
        "environment": ENV_NAME,
        "scope": "StatsForecast Python 3.13 CPU real-data lifecycle smoke",
    ...
    try:
        ...
        data_contract = phase4a_data_contract()
        write_child()
        compile_proc = run(
            [str(RUNTIME), "-m", "py_compile", str(CHILD)],
            timeout=60,
            env={**cpu_env(), "PYTHONPYCACHEPREFIX": f"/tmp/loto-phase4g-child-pycache-{os.getpid()}"},
        )
        (LOCAL_OUT / "child-pycompile.stdout.log").write_text(compile_proc.stdout, encoding="utf-8")
        (LOCAL_OUT / "child-pycompile.stderr.log").write_text(compile_proc.stderr, encoding="utf-8")
        if compile_proc.returncode != 0:
            raise RuntimeError(f"STATSFORECAST_CHILD_PY_COMPILE_FAILED:{compile_proc.stderr.strip()}")
        data = prepare_data(data_contract)
        fit, reload, monitors = execute_lifecycle(data)
        validation = validate_lifecycle(fit, reload, monitors, data)
        if not validation["checks"]["all_critical_checks_pass"]:
            ...
            raise RuntimeError(f"STATSFORECAST_LIFECYCLE_VALIDATION_FAILED:{failed}")

        matched_gpu_pids = sorted(
        ...
        summary.update(
            {
                ...
                "device_policy": {
                    ...
                    "cpu_fallback": False,
                    "reason": "CPU is the intended Phase 4G StatsForecast runtime contract",
                },
                "gpu": {
                    "matched_gpu_pids": matched_gpu_pids,
        ...
        print("=" * 60)
        print("PHASE4G_STATSFORECAST_SMOKE=VERIFIED")
        print(f"HANDOFF_HEAD={head}")
        print(f"SUMMARY={HANDOFF_OUT / 'summary.json'}")
        print(f"REPORT={HANDOFF_OUT / 'PHASE4G_REPORT.md'}")
        print("NEXT_MESSAGE=@GitHub ops/runtime-audit-handoff のPhase 4G結果を確認して次へ進めてください")
        print("=" * 60)
        return 0
        ...
        local_manifest()
        print("=" * 60)
        print("PHASE4G_STATSFORECAST_SMOKE=FAILED")
