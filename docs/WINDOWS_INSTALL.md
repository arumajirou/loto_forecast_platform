...
Native Windows is an explicit repository portability target. The redundant unconditional root `triton==3.5.1` dependency was removed by PR #182, and the locked PyTorch dependency now owns the platform boundary for Triton.
...
`.github/workflows/windows-portability-ci.yml` is the repository's native-Windows portability lane. It runs on `windows-latest` and is intentionally narrower than the Linux/GPU runtime-certification lane.
