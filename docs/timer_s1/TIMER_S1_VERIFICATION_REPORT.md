...
- Focused pytest after final review-readiness hardening: **58 passed**.
- Focused Timer-S1 source coverage: **87%**.
...
- Verified success is limited to `VERIFIED_CPU` and `VERIFIED_GPU` and requires a concrete
  package version.
...
- Manifests require the canonical Timer-S1 core artifact paths, required flags, and kinds.
- Snapshot validation requires exact manifest accounting, size/hash verification, offline mode,
  the exact remote-code allowlist, timezone-aware approved review evidence, and explicit denial of
  environment-secret collection, telemetry/exfiltration, and unsafe deserialization.
