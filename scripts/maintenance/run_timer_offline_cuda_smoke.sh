...
GPU_INDEX='0'
VRAM_RELEASE_TOLERANCE_MIB='128'
...
ready_file.write_text(
    json.dumps({'pid': os.getpid(), 'model_device': model_device, 'input_device': str(x.device)}) + '\n',
    encoding='utf-8',
)
# Keep the provider process observable long enough for external nvidia-smi polling.
time.sleep(3.0)
...
payload = {
    'schema_version': 'timer-base-84m.offline-cuda-smoke.v1',
    'status': 'PASS',
...
# Preserve observability briefly after inference and before normal process exit.
time.sleep(2.0)
PY
...
SNAP="$SNAP" OUT_JSON="$RAW_RUNTIME" READY_FILE="$READY_FILE" SOURCE_HEAD="$SOURCE_HEAD" \
STARTED_AT="$STARTED_AT" EXPECTED_GPU_UUID="$GPU_UUID" \
...
while kill -0 "$PROVIDER_PID" 2>/dev/null; do
  ...
  if [[ -n "$MATCH" ]]; then
    ...
    if [[ "$MATCH_MEM" =~ ^[0-9]+$ ]] && (( MATCH_MEM > VRAM_PEAK_MIB )); then
      VRAM_PEAK_MIB="$MATCH_MEM"
    fi
  fi
  sleep 0.2
done
...
[[ "$VRAM_PEAK_MIB" =~ ^[0-9]+$ ]] || fail 'invalid external peak VRAM value'
(( VRAM_PEAK_MIB > VRAM_BEFORE_MIB )) || fail 'external provider VRAM peak did not exceed baseline'

echo "PASS PROVIDER_PID_VISIBLE_IN_NVIDIA_SMI pid=$PROVIDER_PID"
echo "VRAM_PEAK_MIB=$VRAM_PEAK_MIB"
...
if printf '%s\n' "$AFTER_PROCESSES" | awk -F',' -v p="$PROVIDER_PID" '
  {x=$1; gsub(/[[:space:]]/, "", x); if (x == p) found=1} END{exit found ? 0 : 1}
'; then
...
echo "PASS VRAM_RELEASE before=$VRAM_BEFORE_MIB peak=$VRAM_PEAK_MIB after=$VRAM_AFTER_MIB tolerance=$VRAM_RELEASE_TOLERANCE_MIB"

printf '\n=== 7. Finalize compact CUDA evidence ===\n'
RAW_RUNTIME="$RAW_RUNTIME" FINAL_RUNTIME="$LOCAL_OUT/runtime.json" \
GPU_UUID="$GPU_UUID" PROVIDER_PID="$PROVIDER_PID" PROVIDER_VISIBLE="$PROVIDER_VISIBLE" \
VRAM_BEFORE_MIB="$VRAM_BEFORE_MIB" VRAM_PEAK_MIB="$VRAM_PEAK_MIB" VRAM_AFTER_MIB="$VRAM_AFTER_MIB" \
VRAM_RELEASE_TOLERANCE_MIB="$VRAM_RELEASE_TOLERANCE_MIB" python3 - <<'PY'
import json
import os
from pathlib import Path
...
p['vram_before_mib'] = int(os.environ['VRAM_BEFORE_MIB'])
p['vram_peak_provider_mib'] = int(os.environ['VRAM_PEAK_MIB'])
p['vram_after_mib'] = int(os.environ['VRAM_AFTER_MIB'])
p['vram_release_tolerance_mib'] = int(os.environ['VRAM_RELEASE_TOLERANCE_MIB'])
p['vram_released'] = p['vram_after_mib'] <= p['vram_before_mib'] + p['vram_release_tolerance_mib']
