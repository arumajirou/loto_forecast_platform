...
def inventory_arguments(
    ...
    if fail and unclassified:
        raise RuntimeError(
            f"Unclassified TCN arguments: {unclassified}"
        )
...
def build_combinations(
    ...
    for index, selected in enumerate(
        ...
        combination_id = (
            f"tcn-{index:05d}-"
            f"{sha256_bytes(canonical.encode())[:12]}"
        )
        combinations.append(
            {
...
def model_kwargs(
    ...
    fixed = {
        key: resolve_special_values(value)
        for key, value in config.get("fixed", {}).items()
    }
    search = {
        key: value
        for key, value in combination.items()
        if key != "combination_id"
    }
    ...
    kwargs["alias"] = (
        f"{combination['combination_id']}-seed{seed}"
    )
    return kwargs
...
def verify_properties(
    ...
    for name, requested_value in requested.items():
        ...
        if (
            ...
            and requested_value is None
        ):
            effective_expected = requested.get(
                "batch_size"
            )
            resolution_rule = (
        ...
        rows.append(
            {
                "phase": phase,
                "argument": name,
                "requested": property_value_record(
                    requested_value
                ),
                "effective_expected": (
                    property_value_record(
                        effective_expected
                    )
                ),
                "actual": property_value_record(actual),
                "requested_type": (
                    type(requested_value).__name__
                ),
                "effective_expected_type": (
                    type(effective_expected).__name__
                ),
                "actual_type": (
...
def model_property_snapshot(
    ...
    return {
        ...
        "exogenous_capabilities": {
            "future": bool(
                getattr(model, "EXOGENOUS_FUTR", False)
            ),
            "historical": bool(
                getattr(model, "EXOGENOUS_HIST", False)
            ),
            "static": bool(
                getattr(model, "EXOGENOUS_STAT", False)
            ),
        },
        "multivariate": bool(
            getattr(model, "MULTIVARIATE", False)
        ),
        "recurrent": bool(
            getattr(model, "RECURRENT", False)
        ),
    }
...
def main() -> None:
    ...
    if deterministic_mode == "warn":
        ...
        raise ValueError(
            "runtime.deterministic must be "
            "true, false, or 'warn'; "
            f"received {deterministic_mode!r}"
        )
    ...
    valid_combinations = [
        combination
        for combination in combinations
        if combination["combination_id"] in valid_ids
    ]

    frame = prepare_data(
        ROOT / experiment["data_path"]
    )
    rolling_points = (
        args.rolling_points
        if args.rolling_points > 0
        else int(experiment["rolling_points"])
    )
                    ...
                    prediction_rows.append(
                        {
                            ...
                            "actual": actual,
                            "prediction_raw": raw,
                            "prediction_loaded_raw": (
                                loaded_raw
                            ),
                            "prediction_digit": digit,
                            "prediction_match_after_load": (
                                prediction_match
                            ),
                            "abs_error": abs(
                                actual - raw
    ...
    fitted_properties = pd.DataFrame(
        fitted_property_rows
    )
    ...
    mismatch_count = int(
        (
            ~fitted_properties["matched"].astype(bool)
        ).sum()
        if not fitted_properties.empty
        else 0
    )
    failed_trials = int(
        (trials["status"] != "PASS").sum()
    )
...
- Complete trial execution: {"PASS" if failed_trials == 0 and len(predictions) == total_expected_trials else "FAIL"}
...
"""
    ...
    if fail_ignored and mismatch_count:
        raise SystemExit(
            f"Property mismatch count={mismatch_count}"
        )
    if failed_trials:
        raise SystemExit(
            f"Failed trial count={failed_trials}"
        )
