...
OUTPUT_DIR = ROOT / "artifacts" / "numbers3" / "n1_deepar_only"
...
def main() -> None:
    ...
    for name, factory in model_factories():
        ...
        try:
            ...
            median_columns = [column for column in columns if column.endswith("-median")]

            base_columns = [
                column
                for column in columns
                if not column.endswith("-median") and "-lo-" not in column and "-hi-" not in column
            ]
            ...
            if len(median_columns) == 1:
                ...
                raise RuntimeError(f"Unable to select point forecast column from: {columns}")
    ...
    csv_output = OUTPUT_DIR / "numbers3_n1_deepar_only_results.csv"

    json_output = OUTPUT_DIR / "numbers3_n1_deepar_only_results.json"

    predictions_output = OUTPUT_DIR / "numbers3_n1_deepar_only_predictions.parquet"
    ...
    if failed:
        print("NUMBERS3_N1_DEEPAR_ONLY=PARTIAL")
    else:
        print("NUMBERS3_N1_DEEPAR_ONLY=PASS")
