...
def main():
    ...
    for factor, levels in LEVELS.items():
        for level in levels:
            ...
            rows.append(
                {
                    "design_id": f"{factor}-{cid(row)}",
                    "changed_factor": factor,
                    "changed_level": json.dumps(level),
                    **row,
                }
            )
    ...
    summary = {
        "full_grid_combinations": 384,
        "screening_configurations": len(rows),
        "screening_fits": len(rows) * 3 * 20,
    }
    (a.output_dir / "design_summary.json").write_text(
