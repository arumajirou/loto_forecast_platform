    ...
    *,
    ...
    if proc.returncode == 0:
        ...
        if not current:
            ...
            print(
                "RUFF_FORMAT_BASELINE_TOOL_ERROR=unable_to_parse_failure",
                file=sys.stderr,
            )
            return 2
    else:
        print(combined, file=sys.stderr)
        print(
            f"RUFF_FORMAT_BASELINE_TOOL_ERROR=ruff_exit_{proc.returncode}",
            file=sys.stderr,
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Reject Ruff format debt not present in "
            "the immutable repository baseline"
        )
    )
    parser.add_argument(
        "--baseline",
