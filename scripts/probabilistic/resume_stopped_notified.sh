...
uv run python scripts/probabilistic/verify_acceleration.py \
  --config configs/probabilistic/native_resume_stopped_notified.yaml

mkdir -p artifacts/probabilistic-notifications
STAMP="$(date +%Y%m%d-%H%M%S)"
LOG="$ROOT/artifacts/probabilistic-notifications/resume-${STAMP}.log"
RUN_DIR="$ROOT/runs/probabilistic-native/ppl-20260803-140548-c65bafdb"
...
uv run python scripts/probabilistic/progress_dashboard.py \
  --run-dir "$RUN_DIR" --runner-pid "$RUNNER_PID" --interval 2
DASHBOARD_STATUS=$?
wait "$RUNNER_PID"
RUNNER_STATUS=$?
set -e
