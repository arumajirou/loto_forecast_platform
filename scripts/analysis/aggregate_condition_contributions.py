    ...
    for filename, comparison_group in comparisons:
        ...
        if not result.empty:
            stable_frames.append(
                result[
                    (result["cluster_ci95_low"] > 0)
                    & result["all_seeds_positive"]
                    & (result["positive_fold_rate"] >= 0.6)
                    & (result["front_half_contribution"] > 0)
                    & (result["back_half_contribution"] > 0)
                    & (result["pvalue_holm"] < 0.05)
                ].copy()
            )
    quality_columns = [
        column
        for column in ("probability_std", "probability_unique_count", "probability_quality_pass")
        if column in frame
    ]
    quality = (
        frame.groupby(["model_id", "condition", "feature_group"], dropna=False)[quality_columns]
        .agg(["mean", "min", "max"])
        .reset_index()
        if quality_columns
        else pd.DataFrame()
