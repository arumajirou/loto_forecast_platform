...
def _runner_factory(
    ...
    *,
    ...
    quality_git: str = COMMIT,
    focused_count: int = gate.EXPECTED_FOCUSED_TESTS,
    target_version: str = gate.TARGET_VERSION,
    ...
    def runner(command, cwd, stdout_path, stderr_path):
        ...
        if name == "quality":
            ...
            report = {
                "quality_run_id": directory.name,
                "status": "VERIFIED" if quality_code == 0 else "FAILED_RUFF_LINT",
                "formal_success": quality_code == 0,
                "evidence_directory": str(directory),
                "expected_git_sha": COMMIT,
                "git_commit": quality_git,
                "git_preflight": _probe(quality_git),
                "git_postflight": _probe(quality_git),
                "focused_junit": {
                    "tests": focused_count,
                    "failures": 0,
                    "errors": 0,
            ...
            _seal(directory, "QUALITY_REPORT.json", report, "quality_run_id")
            if tamper_quality:
        ...
        if name == "target":
            ...
            _seal(directory, "OPERATOR_REPORT.json", report, "operator_run_id")
            stdout_path.write_text(json.dumps(report), encoding="utf-8")
...
def test_quality_git_identity_mismatch_is_rejected(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    runner, calls = _runner_factory(root, quality_git="b" * 40)
    report, code = _execute(root, runner)
    assert code == 2
    assert report["status"] == "FAILED_QUALITY_GATE"
    assert "Git identity mismatch" in str(report["error"])
    assert calls == ["quality"]


def test_quality_focused_count_mismatch_is_rejected(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    runner, calls = _runner_factory(root, focused_count=gate.EXPECTED_FOCUSED_TESTS - 1)
    report, code = _execute(root, runner)
    assert code == 2
    assert report["status"] == "FAILED_QUALITY_GATE"
    assert "focused JUnit" in str(report["error"])
    assert calls == ["quality"]
...
def test_target_version_mismatch_is_rejected(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    runner, calls = _runner_factory(root, target_version="1.5.0")
    report, code = _execute(root, runner)
    assert code == 2
    assert report["status"] == "FAILED_TARGET_CERTIFICATION"
    assert "installed-version" in str(report["error"])
    assert calls == ["quality", "target"]
