...
def test_inventory_and_snapshot_roundtrip(tmp_path: Path) -> None:
    ...
    assert len(files) == len(SOURCE_PATHS) == 10
...
def test_verify_working_source_checks_git_and_digest(
    tmp_path: Path,
) -> None:
    repository = _repository(tmp_path / "repo")
    files = collect_source_inventory(repository)
...
def test_git_checkout_rejects_dirty_or_detached(
    tmp_path: Path,
) -> None:
    repository = _repository(tmp_path / "repo")
