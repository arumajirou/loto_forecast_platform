...
def test_all_game_local_acquisition_and_lineage(tmp_path):
    ...
    result = acquire_and_build_many(
        games="all", output_dir=tmp_path / "out", source_files=sources, continue_on_error=False
    )
    assert result["status"] == "SUCCEEDED"
    assert set(result["successful_games"]) == set(specs)
    for game in specs:
        report = json.loads(
            (tmp_path / "out" / game / "acquisition_report.json").read_text(encoding="utf-8")
        )
...
def test_schedule_plan_is_draw_aware():
    ...
    plan = build_schedule_plan(
        "loto7,numbers3", now=now, policy=SchedulePolicy(run_hour=20, run_minute=30)
    )
...
def test_api_exposes_runs_events_resources_and_games(tmp_path, monkeypatch):
    ...
    (run_dir / "research_summary.json").write_text(
        json.dumps({"run_id": "demo-run", "status": "SUCCEEDED"})
    )
    ...
    plan = client.post(
        "/api/v2/model-plan",
        json={"model_name": "AutoNHITS", "h": 1, "backend": "optuna", "num_samples": 10},
    )
