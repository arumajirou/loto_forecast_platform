...
def _write_numbers3_snapshot(
    ...
    frame = pd.DataFrame(
        {
            "game_id": ["numbers3"] * 130,
            "draw_no": list(range(1, 131)),
            "ds": pd.date_range(
                "2025-01-01",
                periods=130,
                freq="D",
            ),
            "N1": [value % 10 for value in range(130)],
            "N2": [(value + 3) % 10 for value in range(130)],
            "N3": [(value + 6) % 10 for value in range(130)],
        }
    )
...
def _fake_predictor(
    ...
    prediction = tuple((float(values[-1]) + 0.25,) for values in request.series)
    ...
    return TimerResponse(
        ...
        observed_source_head=request.observed_source_head,
        ...
        input_series_sha256_f32=hashlib.sha256(input_raw).hexdigest(),
        prediction_sha256_f32=hashlib.sha256(prediction_raw).hexdigest(),
...
def test_context_reader_excludes_target_actual(
    ...
    _write_numbers3_snapshot(snapshot)

    reader = DevelopmentSnapshotReader(snapshot)
    ...
    assert context["draw_no"].tolist() == list(range(25, 121))

    assert 121 not in set(context["draw_no"])
...
def test_baseline_bundle_seals_all_predictions_before_actual(
    ...
    snapshot = tmp_path / "snapshot"
    frame = _write_numbers3_snapshot(snapshot)
    ...
    result = run_baseline_target_bundle(
        reader=reader,
        output_dir=(tmp_path / "baseline-target"),
    ...
    verify_target_bundle(result)

    assert len(result.seals) == 9

    assert all(
        sealed.sealed_at_utc <= result.actual_reveal.actual_read_at_utc for sealed in result.seals
    )

    score = json.loads(result.score_path.read_text(encoding="utf-8"))

    assert score["all_predictions_sealed_before_actual"] is True
...
def test_timer_bundle_seals_layouts_before_actual(
    ...
    result = run_timer_target_bundle(
        reader=reader,
        output_dir=(tmp_path / "timer-target"),
        ...
        specs=(
            TimerPredictionSpec(
                candidate_id="timer-position-univariate",
                target_layout="position_univariate",
                seed=42,
                requested_device="cpu",
            ),
            TimerPredictionSpec(
                candidate_id="timer-position-panel",
                target_layout="position_panel_batched_univariate",
                seed=1729,
                requested_device="cpu",
            ),
        ),
    ...
    assert {item["target_layout"] for item in score["scores"]} == {
        "position_univariate",
