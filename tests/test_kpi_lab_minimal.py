...
def test_proposal_requires_point_method() -> None:
    proposal, rejection = validate_proposal({"window": 10}, source="llm", proposal_id="t4")
    assert proposal is None
...
def test_llm_unavailable_is_typed() -> None:
    ...
    proposer = LlmProposer(endpoint="http://127.0.0.1:1/v1/chat/completions", timeout_seconds=0.5)
...
def _honest_builder(game: str, tolerance: int):
    def builder(build_draws, n_tickets, seed):
        pool, _ = reference_pool(game, n_tickets=n_tickets, construction="random_legal", seed=seed)
        return pool
...
def _overpowered_builder(game: str, pool_size: int):
    ...
    def builder(build_draws, n_tickets, seed):
        pool, _ = reference_pool(game, n_tickets=pool_size, construction="offset_lattice")
        return pool
