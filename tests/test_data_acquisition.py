...
def _sample(path: Path) -> None:
    ...
    for draw in range(1, 13):
        ...
        rows.append({"回号":draw,"抽選日":f"2026-01-{draw:02d}",**{f"本数字{i+1}":n for i,n in enumerate(nums[:7])}})
    pd.DataFrame(rows).to_csv(path,index=False,encoding="cp932")


def test_flexible_parse_and_integrated_bundle(tmp_path):
    raw=tmp_path/'raw.csv'; _sample(raw)
    ...
    assert {'draw_features','occurrence_features','canonical_loto7','position_loto7','candidate_features_v2'} <= set(names)


def test_legacy_features_cover_original_windows(tmp_path):
    raw=tmp_path/'raw.csv'; _sample(raw)
    from loto.data.parser import parse_file
    spec=get_lottery_spec('loto7'); normalized,_=parse_file(raw,spec)
    draw=make_draw_features(normalized,spec)
    occurrence=make_occurrence_features(normalized,spec)
    assert {'num_sum','num_prime_count','num_consecutive_pairs','ending_entropy'} <= set(draw.columns)
    assert {'freq_last_5','freq_last_10','freq_last_20','freq_last_50','candidate_mod10'} <= set(occurrence.columns)
