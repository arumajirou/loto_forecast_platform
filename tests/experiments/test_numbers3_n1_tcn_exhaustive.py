...
SCRIPT = ROOT / "scripts" / "experiments" / "run_numbers3_n1_tcn_exhaustive.py"
CONFIG = ROOT / "configs" / "experiments" / "numbers3_n1_tcn_exhaustive.yaml"
...
def test_combinations_are_complete_and_unique(
    ...
    assert len({row["combination_id"] for row in combinations}) == expected
...
def test_all_configured_search_arguments_exist(
    ...
    unknown = sorted(set(config["search"]) - names)
    assert not unknown
...
def test_all_fixed_arguments_exist(
    ...
    unknown = sorted(set(config["fixed"]) - names - runtime_keys)
    assert not unknown
...
def test_constructor_and_property_reflection(
    ...
    combination = module.build_combinations(config)[0]
...
def test_valid_batch_size_none_resolves_to_batch_size(
    ...
    row = next(item for item in rows if item["argument"] == "valid_batch_size")

    assert row["matched"] is True
    assert row["resolution_rule"] == "None resolves to batch_size"
    assert row["effective_expected"] == (module.property_value_record(kwargs["batch_size"]))
    assert row["actual"] == (module.property_value_record(kwargs["batch_size"]))
