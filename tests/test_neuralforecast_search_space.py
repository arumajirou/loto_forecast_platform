...
class Integer:
    def __init__(self, low, high): self.lower, self.upper = low, high
class Categorical:
    def __init__(self, values): self.categories = values
...
def test_optuna_profile_is_partial_and_detects_branch():
    def config(trial):
        ...
        if family == "b": result["width"] = trial.suggest_int("width", 16, 64, step=16)
        return result
