...
from loto.coverage.auto_research import (
    _evaluate_general,
    _expand_space,
    _greedy_general,
    _legalize,
...
def test_greedy_general_reaches_target():
    ...
    selected, trace = _greedy_general(
        actual, pool, target=0.9, tolerance=1, max_candidates=5, diversity=0
    )
...
def test_expand_space_cartesian():
    rows = list(_expand_space({"alpha": [0.1, 1], "window": [20, 50]}))
    assert len(rows) == 4
