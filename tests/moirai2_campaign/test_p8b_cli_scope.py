...
def test_candidate_cli_is_non_destructive_and_uses_uv_lock() -> None:
    source = (ROOT / "scripts" / "generate_moirai2_lock_candidate.py").read_text(encoding="utf-8")
...
def test_install_cli_requires_explicit_approval_guards() -> None:
    source = (ROOT / "scripts" / "install_reviewed_moirai2_lock.py").read_text(encoding="utf-8")
...
def test_runtime_preflight_requires_installed_review() -> None:
    source = (ROOT / "src" / "loto" / "moirai2_campaign" / "runtime_preflight.py").read_text(
        encoding="utf-8"
    )
...
def test_campaign_passes_runtime_lane_to_preflight() -> None:
    source = (ROOT / "scripts" / "run_moirai2_runtime_campaign.py").read_text(encoding="utf-8")
