...
def test_artifact_summary_for_directory(
    ...
    (artifact / "configuration.pkl").write_bytes(
        b"configuration"
    )
    (artifact / "dataset.pkl").write_bytes(
        b"dataset"
    )
    (artifact / "model.ckpt").write_bytes(
        b"checkpoint"
    )
    ...
    assert summary["size_bytes"] == (
        len(b"configuration")
        + len(b"dataset")
        + len(b"checkpoint")
    )
    assert summary["sha256"] == expected_directory_hash(
        artifact
    )
    assert [
        row["path"]
