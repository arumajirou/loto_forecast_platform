...
def test_identity_hash_and_campaign_require_all_models() -> None:
    ...
    with pytest.raises(ValidationError):
        P10CampaignConfig(partition=PARTITION, ensembles=(_ensemble('NaiveEnsembleModel'),
            ), conformal_models=(_conformal('ConformalNaiveModel'), _conformal('ConformalQRModel')))

def test_temporal_partition_and_quantile_contracts_fail_closed() -> None:
    with pytest.raises(ValidationError):
        TemporalPartition(train_end=10, calibration_start=9, calibration_end=12,
            evaluation_start=12, evaluation_end=14)
    with pytest.raises(ValidationError):
        ConformalConfig(public_name='ConformalNaiveModel', base_model_id='a',
            quantiles=(0.1, 0.5, 0.8))
    with pytest.raises(ValidationError):
        ConformalConfig(public_name='ConformalNaiveModel', base_model_id='a',
            quantiles=(0.1, 0.5, 0.9), num_samples=10, predict_likelihood_parameters=True)
...
def test_unavailable_base_is_retained_as_dependency_failure() -> None:
    unavailable = BaseModelEvidence(model_id='b', model_family='torch', available=False,
        failure_class='ImportError', failure_message='optional dependency missing')
...
def test_no_silent_drop_argument_ledger_is_scoped() -> None:
    ...
    class RuntimeModel:
        ...
        def predict(self, n: int, num_samples: int=1) -> None:
            del n, num_samples
    config = _ensemble('NaiveEnsembleModel').model_copy(update={'constructor_args': {'alpha': 2.0},
        'fit_args': {'verbose': True}, 'predict_args': {'num_samples': 1}})
    plan = build_ensemble_plan(config, {'a': _base('a'), 'b': _base('b')},
        constructor=RuntimeModel, fit_method=RuntimeModel.fit, predict_method=RuntimeModel.predict)
    assert {(item.phase, item.argument) for item in plan.argument_ledger} == {('constructor',
        'alpha'), ('fit', 'verbose'), ('predict', 'num_samples')}
    bad = config.model_copy(update={'predict_args': {'unknown': 1}})
    with pytest.raises(P10ContractError, match='rejected predict arguments'):
        build_ensemble_plan(bad, {'a': _base('a'), 'b': _base('b')},
