...
def test_regression_stacking_rejects_leakage_and_incomplete_rows() -> None:
    training = tuple((_point(model, seed=seed, fold_id=0, origin=12, target=13,
        position='N1') for seed in (1, 7) for model in ('a', 'b')))
    evaluation = tuple((_point('ensemble', seed=seed, fold_id=0, origin=15, target=16,
        position='N1') for seed in (1, 7)))
    evidence = StackingEvidence(training_records=training, evaluation_records=evaluation,
        observed_base_model_ids=('a', 'b'), observed_seeds=(1, 7), observed_fold_ids=(0,))
    report = certify_stacking_evidence(evidence, expected_base_model_ids=('a', 'b'),
        expected_seeds=(1, 7), expected_fold_ids=(0,), partition=PARTITION)
    assert report['training_record_count'] == 4
    leaking = evidence.model_copy(update={'training_records': training + (_point('a', seed=1,
        fold_id=0, origin=15, target=16, position='N1'),)})
    with pytest.raises(CertificationError, match='evaluation-period'):
        certify_stacking_evidence(leaking, expected_base_model_ids=('a', 'b'),
            expected_seeds=(1, 7), expected_fold_ids=(0,), partition=PARTITION)
    ...
    with pytest.raises(CertificationError, match='every base model'):
        certify_stacking_evidence(incomplete, expected_base_model_ids=('a', 'b'),
            expected_seeds=(1, 7), expected_fold_ids=(0,), partition=PARTITION)
...
def test_interval_metrics_and_quantile_non_crossing() -> None:
    ...
    cert = certify_conformal_quantiles(_conformal('ConformalNaiveModel'), PARTITION, actual,
        {0.1: low, 0.5: median, 0.9: high}, base_median_prediction=median)
    ...
    with pytest.raises(CertificationError, match='cross'):
        certify_conformal_quantiles(_conformal('ConformalNaiveModel'), PARTITION, actual,
            {0.1: high, 0.5: median, 0.9: low}, base_median_prediction=median)
...
def test_conformal_median_parity_and_shape_are_fail_closed() -> None:
    ...
    with pytest.raises(CertificationError, match='base median'):
        certify_conformal_quantiles(_conformal('ConformalNaiveModel'), PARTITION, actual,
            predictions, base_median_prediction=np.zeros((2, 2)))
    with pytest.raises(CertificationError, match='shapes differ'):
        compute_interval_metrics(actual, np.zeros((1, 2)), np.ones((2, 2)),
