WITH base AS (
    SELECT
        ...
        ) AS expected_roll_mean_3,
        AVG(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_mean_7,
        AVG(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 14 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_mean_14,
        AVG(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 28 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_mean_28,

        STDDEV_SAMP(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_std_7,
        STDDEV_SAMP(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 14 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_std_14,
        STDDEV_SAMP(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 28 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_std_28,

        MIN(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_min_7,
        MAX(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_max_7,
        MIN(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 14 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_min_14,
        MAX(y) OVER (
            PARTITION BY unique_id ORDER BY ds
            ROWS BETWEEN 14 PRECEDING AND 1 PRECEDING
        ) AS expected_roll_max_14,
        ...
        ROW_NUMBER() OVER w - 1 AS expected_row_no,
        AVG(y) OVER (
            PARTITION BY unique_id
        ) AS full_sample_mean
...
dates AS (
    SELECT
        (SELECT MAX(ds)::date
         FROM exog.loto_y_ts_exog
         WHERE loto='loto7' AND ts_type='raw') AS source_last_date,
        (SELECT MAX(ds)::date
         FROM dataset.loto_y_ts
         WHERE loto='loto7' AND ts_type='raw') AS target_last_date
),
metrics AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE
            ...
            OR (hist_roll_mean_7 IS NOT NULL AND expected_roll_mean_7 IS NOT NULL
        ...
        COUNT(*) FILTER (WHERE
            ...
            OR (hist_roll_mean_14 IS NOT NULL AND expected_roll_mean_14 IS NOT NULL
        ...
        COUNT(*) FILTER (WHERE
            ...
            OR (hist_roll_mean_28 IS NOT NULL AND expected_roll_mean_28 IS NOT NULL
        ...
        COUNT(*) FILTER (WHERE
            ...
            OR (hist_expanding_mean IS NOT NULL AND expected_expanding_mean IS NOT NULL
                AND ABS(hist_expanding_mean - expected_expanding_mean) > 1e-9)
        ) AS expanding_mean_mismatches,

        COUNT(*) FILTER (WHERE feat_year <> EXTRACT(YEAR FROM ds)::bigint)
            AS calendar_year_mismatches,
        COUNT(*) FILTER (WHERE feat_month <> EXTRACT(MONTH FROM ds)::bigint)
            AS calendar_month_mismatches,
        COUNT(*) FILTER (WHERE feat_day <> EXTRACT(DAY FROM ds)::bigint)
        ...
        COUNT(*) FILTER (WHERE
            ...
            OR (hist_roll_std_7 IS NOT NULL AND expected_roll_std_7 IS NOT NULL
                AND ABS(hist_roll_std_7 - expected_roll_std_7) > 1e-9)
        ) AS candidate_roll_std7_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_roll_std_14 IS NULL) <> (expected_roll_std_14 IS NULL)
            OR (hist_roll_std_14 IS NOT NULL AND expected_roll_std_14 IS NOT NULL
                AND ABS(hist_roll_std_14 - expected_roll_std_14) > 1e-9)
        ) AS candidate_roll_std14_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_roll_std_28 IS NULL) <> (expected_roll_std_28 IS NULL)
            OR (hist_roll_std_28 IS NOT NULL AND expected_roll_std_28 IS NOT NULL
                AND ABS(hist_roll_std_28 - expected_roll_std_28) > 1e-9)
        ) AS candidate_roll_std28_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_roll_min_7 IS NULL) <> (expected_roll_min_7 IS NULL)
            OR (hist_roll_min_7 IS NOT NULL AND expected_roll_min_7 IS NOT NULL
                AND ABS(hist_roll_min_7 - expected_roll_min_7) > 1e-9)
        ) AS candidate_roll_min7_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_roll_max_7 IS NULL) <> (expected_roll_max_7 IS NULL)
            OR (hist_roll_max_7 IS NOT NULL AND expected_roll_max_7 IS NOT NULL
                AND ABS(hist_roll_max_7 - expected_roll_max_7) > 1e-9)
        ) AS candidate_roll_max7_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_roll_min_14 IS NULL) <> (expected_roll_min_14 IS NULL)
            OR (hist_roll_min_14 IS NOT NULL AND expected_roll_min_14 IS NOT NULL
                AND ABS(hist_roll_min_14 - expected_roll_min_14) > 1e-9)
        ) AS candidate_roll_min14_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_roll_max_14 IS NULL) <> (expected_roll_max_14 IS NULL)
            OR (hist_roll_max_14 IS NOT NULL AND expected_roll_max_14 IS NOT NULL
                AND ABS(hist_roll_max_14 - expected_roll_max_14) > 1e-9)
        ) AS candidate_roll_max14_mismatches,
        COUNT(*) FILTER (WHERE
            (hist_expanding_std IS NULL) <> (expected_expanding_std IS NULL)
            OR (hist_expanding_std IS NOT NULL AND expected_expanding_std IS NOT NULL
                AND ABS(hist_expanding_std - expected_expanding_std) > 1e-9)
        ) AS candidate_expanding_std_mismatches,
        COUNT(*) FILTER (
            WHERE feat_days_since_first <> (ds::date - first_ds)
        ) AS candidate_days_since_first_mismatches,
        COUNT(*) FILTER (
            WHERE feat_row_no_in_group <> expected_row_no
        ) AS candidate_row_no_mismatches,
...
SELECT
    ...
    COUNT(*) FILTER (
        WHERE b.ds::date = dates.target_last_date
    ) AS latest_rows,
    COUNT(DISTINCT b.unique_id) FILTER (
        WHERE b.ds::date = dates.target_last_date
    ) AS latest_series,
    ...
    m.candidate_roll_std7_mismatches,
    m.candidate_roll_std14_mismatches,
    m.candidate_roll_std28_mismatches,
    m.candidate_roll_min7_mismatches,
    m.candidate_roll_max7_mismatches,
    m.candidate_roll_min14_mismatches,
    m.candidate_roll_max14_mismatches,
    m.candidate_expanding_std_mismatches,
    m.candidate_days_since_first_mismatches,
    m.candidate_row_no_mismatches,
