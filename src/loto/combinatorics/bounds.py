...
def neighbourhood_size(
    ticket: Sequence[int], geometry: GameGeometry, tolerance: int = 1
) -> int:
...
def max_neighbourhood_size(game: str, tolerance: int = 1) -> tuple[int, tuple[int, ...]]:
    ...
    best = 0
    best_ticket: tuple[int, ...] = tuple(
        geometry.value_min + i for i in range(geometry.positions)
    )
...
def feasibility_bound(
    ...
    *,
    ...
    mean_neigh = mean_neighbourhood_size(
        game, tolerance, samples=mean_samples, seed=seed
    )
