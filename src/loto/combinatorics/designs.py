...
def greedy_uniform_pool(
    ...
    *,
    ...
    extra, _ = random_legal_pool(
        game, n_tickets=max(0, candidate_budget - len(lattice)), seed=seed
    )
    ...
    *,
    ...
    if construction not in REFERENCE_CONSTRUCTIONS:
        raise ValueError(
            ...
            f"available={sorted(REFERENCE_CONSTRUCTIONS)}"
        )
    if construction == "greedy_uniform":
        return greedy_uniform_pool(
            game, n_tickets=n_tickets, tolerance=tolerance, seed=seed
        )
    ...
    return multiplicity_augmented_pool(
        game, tolerance=tolerance, limit=n_tickets, seed=seed
    )
