...
def coverage_mask(targets: np.ndarray, ticket: Sequence[int], tolerance: int) -> np.ndarray:
...
def greedy_min_cover(
    ...
    *,
    ...
    result = greedy_max_coverage(targets, pool, budget=cap, tolerance=tolerance, record_trace=True)
