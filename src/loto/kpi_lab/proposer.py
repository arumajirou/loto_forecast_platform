...
PARAMETER_SPACE: dict[str, ParameterSpec] = {
    ...
    "selection": ParameterSpec(kind="choice", choices=("greedy_max", "greedy_min_cover", "topk")),
...
def validate_proposal(
    ...
    unknown = [key for key in raw if key not in PARAMETER_SPACE and key != "rationale"]
    if unknown:
    ...
    def _allowed_description(self) -> str:
        ...
        for key, spec in sorted(PARAMETER_SPACE.items()):
            if spec.kind == "choice":
                ...
                lines.append(f"- {key}: {spec.kind} in [{spec.minimum}, {spec.maximum}]")
        return "\n".join(lines)
        ...
        **_: Any,
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": self.SYSTEM_PROMPT.format(allowed=self._allowed_description()),
                },
