...
class KpiLab:
    ...
    def __init__(self, config: LabConfig) -> None:
        ...
        self.ledger = ExperimentLedger(self.output_dir / "ledger.jsonl", session_id=self.session_id)
    ...
    def run(self, draws: np.ndarray) -> LabReport:
        ...
        if self.config.llm_endpoint:
            proposer = LlmProposer(endpoint=self.config.llm_endpoint, model=self.config.llm_model)
        else:
            proposer = grid
        ...
        for index in range(budget.max_experiments):
            ...
            try:
                ...
                comparison = compare_arms(reference, model, calibration, tolerance=kpi.tolerance)
                delta = comparison.delta
                efficiency = coverage_efficiency(
                    achieved_coverage=model.sealed_coverage,
                    n_tickets=model.n_tickets,
                    lower_bound_tickets_at_coverage=bound.lower_bound_for(model.sealed_coverage),
                )
                status = "OK"
                error = None
                consecutive_failures = 0
        ...
        confirmation = compare_arms(reference, confirm_model, sealed_draws, tolerance=kpi.tolerance)
        eproc.update_paired(list(confirmation.model_hits), list(confirmation.reference_hits))
        ...
        beat_reference = confirmation.delta > kpi.min_arm_delta and decision.rejected_null
        if beat_reference:
            reason = (
