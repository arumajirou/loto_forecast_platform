...
class PaceGate:
    ...
    def _lambda(self) -> float:
        if self.n < 2:
            return 0.5
        mu = self._sum / self.n
        var = max(self._sum_sq / self.n - mu * mu, 1e-6)
    ...
    def update(self, hits_candidate: np.ndarray, hits_champion: np.ndarray) -> dict:
        a = np.asarray(hits_candidate, bool)
        b = np.asarray(hits_champion, bool)
        if a.shape != b.shape:
            raise ValueError("paired hit vectors must align")
        diff = float(a.mean() - b.mean())
        u = (diff + 1) / 2
        lam = self._lambda()
        factor = 1 + lam * (u - 0.5)
        if factor <= 0:
            raise AssertionError("betting factor must be positive")
    ...
    def decision(self) -> str:
        if self.n < self.config.min_draws:
            return "COLLECTING"
        if self.log_e >= np.log(1 / self.config.alpha):
            return "ACCEPT"
        return "INCONCLUSIVE"
