"""Shared command-line adapter for the unified evaluation campaign."""
...
def _load_frames(args: argparse.Namespace, games: tuple[str, ...]) -> dict[str, pd.DataFrame]:
    ...
    for game in games:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"missing input CSV for {game}: {path}")
        frames[game] = pd.read_csv(path)
    return frames


def add_campaign_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Add the same campaign options to standalone and ``loto3`` parsers."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    return add_campaign_arguments(parser)


def run_campaign_args(args: argparse.Namespace) -> int:
    """Execute parsed campaign arguments and print one machine-readable result."""
...
def main(argv: list[str] | None = None) -> int:
    return run_campaign_args(build_parser().parse_args(argv))
