...
def run_trusted_vertical_slice(
    input_csv: str | Path,
    output_dir: str | Path,
    *,
    secret: bytes,
    ...
    registry = Registry(output / "registry.sqlite3")
    publisher = EventPublisher(output / "events.jsonl")
    platform_registry = PlatformRegistry(
        os.environ.get("LOTO_REGISTRY_URL", str(output / "platform.sqlite3"))
    )
    ...
    (output / "resource_evidence.json").write_text(
    ...
    def stage(name, status="SUCCEEDED", payload=None):
        ...
        platform_registry.record_task(
            run_id, name, task_status, output_uri=body.get("artifact_uri")
        )
        platform_registry.update_run(
            run_id,
            status="RUNNING" if task_status not in {"FAILED"} else "FAILED",
            current_stage=name,
            error=body if task_status == "FAILED" else None,
        )
        publisher.publish(
            "run.stage.changed",
            {"run_id": run_id, "stage": name, "status": status, "payload": body},
        )
    ...
    stage("INGEST", payload={"rows": len(raw)})
    master, manifest = canonicalize_loto7(raw, source=str(input_csv))
    stage("VALIDATE", payload={"data_version": manifest.data_version})
    master.to_csv(output / "canonical.csv", index=False)
    save_manifest(manifest, output / "dataset_manifest.json")
    stage("CANONICALIZE", payload={"sha256": manifest.sha256})
    features = build_candidate_features(master, windows=windows)
    feat_manifest = feature_manifest(features, manifest.data_version, windows)
    features.to_csv(output / "candidate_features.csv", index=False)
    (output / "feature_manifest.json").write_text(
        json.dumps(feat_manifest.model_dump(mode="json"), indent=2), encoding="utf-8"
    )
    stage(
        "BUILD_FEATURES",
        payload={"feature_set_id": feat_manifest.feature_set_id, "rows": len(features)},
    )
    ...
    for idx in range(start, len(master)):
        history = master.iloc[:idx]
        current = master.iloc[idx]
        hist_candidates = to_candidate_table(history)
        ...
        preds_uniform.append(du)
        preds_frequency.append(df)
        targets.append(_candidate_targets(actual))
        probs_u.append(u["probability"].to_numpy())
        probs_f.append(f["probability"].to_numpy())
    if actuals:
        act = np.asarray(actuals)
        pu = np.asarray(preds_uniform)
        pf = np.asarray(preds_frequency)
        y = np.asarray(targets)
        metrics_u = evaluate_draws(act, pu) | {
            "brier": brier_score(y, np.asarray(probs_u)),
            "log_loss": log_loss(y, np.asarray(probs_u)),
        }
        metrics_f = evaluate_draws(act, pf) | {
            "brier": brier_score(y, np.asarray(probs_f)),
            "log_loss": log_loss(y, np.asarray(probs_f)),
        }
    else:
        metrics_u = metrics_f = {
            "mean_hits_at_7": 0.0,
    ...
    champion = (
        "frequency"
        if (
            metrics_f["mean_hits_at_7"] > metrics_u["mean_hits_at_7"]
            and metrics_f["brier"] <= metrics_u["brier"] * 1.02
        )
        else "uniform"
    )
    stage("TRAIN", payload={"models": ["uniform", "frequency"], "selected": champion})
    stage("CALIBRATE", payload={"method": "identity-v1"})

    all_candidates = to_candidate_table(master)
    query = build_next_candidate_features(master, windows=windows)
    model = (
        FrequencyCandidateAdapter().fit(all_candidates)
        if champion == "frequency"
        else UniformCandidateAdapter().fit(all_candidates)
    )
    pred = model.predict(query)
    pos = PositionFrequencyAdapter().fit(master).predict_matrix()
    combos = decode_hybrid(pred["rank_score"].to_numpy(), pos, top_k=20)
    ...
    stage("EVALUATE", payload=report)

    created = datetime.now(UTC)
    draw_time = max(
        created + timedelta(minutes=1),
        master["draw_date"].max().to_pydatetime() + timedelta(days=7),
    )
    forecast = ForecastPackage(
        ...
        feature_set_id=feat_manifest.feature_set_id,
        created_at=created,
        draw_time=draw_time,
        combination=combos[0],
        candidates=[
            CandidateProbability(
                candidate_number=int(r.candidate_number),
    ...
    sealed = seal_payload(payload, secret)
    verified = verify_seal(sealed, secret)
    (output / "forecast.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (output / "forecast.sealed.json").write_text(
    ...
    publisher.publish(
        "forecast.sealed",
        {"run_id": run_id, "forecast_id": forecast.forecast_id, "verified": verified},
    )
    if FORECAST_SEALED is not None:
        FORECAST_SEALED.inc()
    registry.record_forecast(forecast.forecast_id, run_id, sealed, verified)
    platform_registry.register_forecast(
        forecast.forecast_id, run_id, forecast.draw_id, sealed, verified
    )
    ...
    bridge = MlflowBridge(
        os.environ.get("MLFLOW_TRACKING_URI", "http://127.0.0.1:5050"),
        os.environ.get("MLFLOW_EXPERIMENT_NAME", "loto-trusted-vertical-slice"),
    )
    flat_metrics = {f"{champion}_{k}": float(v) for k, v in report[champion].items()}
    mlflow_status = bridge.record_run(
        run_id,
        {
            "champion": champion,
            "data_version": manifest.data_version,
    ...
    (output / "mlflow_status.json").write_text(
        json.dumps(mlflow_status, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    ...
    artifact_index = {
        Path(p).name: store.put_file(p, namespace=run_id)
        for p in artifacts + [output / "release_bundle.json"]
    }
    (output / "artifact_index.json").write_text(
        json.dumps(artifact_index, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    platform_registry.register_model(
        model.model_id,
        champion,
        artifact_index["release_bundle.json"]["uri"],
        report[champion],
        {"run_id": run_id, "data_version": manifest.data_version},
    )
    platform_registry.update_run(
        run_id, status="SUCCEEDED", current_stage="REGISTER", release_id=release_id
    )
    platform_registry.audit(
        "system",
        "complete",
        "run",
        run_id,
        "trusted vertical slice completed",
