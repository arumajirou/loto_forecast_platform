"""Strict target-host runtime contracts for AutoSegRNN certification."""
...
class AutoSegRNNRuntimeRequest(StrictRuntimeModel):
    """Immutable request for one fixed AutoSegRNN runtime lane."""
    ...
    dropout: float = Field(default=0.1, ge=0.0, lt=0.5)
    ...
    def validate_runtime_policy(self) -> AutoSegRNNRuntimeRequest:
...
class AutoSegRNNWorkerResponse(StrictRuntimeModel):
    ...
    @model_validator(mode="after")
    def validate_status(self) -> AutoSegRNNWorkerResponse:
...
def canonical_request_payload(request: AutoSegRNNRuntimeRequest) -> dict[str, Any]:
    return request.model_dump(mode="json")


def canonical_request_sha256(request: AutoSegRNNRuntimeRequest) -> str:
    encoded = json.dumps(
...
def load_runtime_request(path: Path) -> AutoSegRNNRuntimeRequest:
    return AutoSegRNNRuntimeRequest.model_validate_json(path.read_text(encoding="utf-8"))


def load_worker_response(path: Path) -> AutoSegRNNWorkerResponse:
    return AutoSegRNNWorkerResponse.model_validate_json(path.read_text(encoding="utf-8"))
