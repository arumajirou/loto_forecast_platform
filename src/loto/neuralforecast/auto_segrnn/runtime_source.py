"""Git and source-byte identity helpers for AutoSegRNN runtime evidence."""
...
def verify_git_checkout(
    ...
    *,
    ...
    status = command_runner(root, ["status", "--porcelain=v1", "--untracked-files=all"])
    if status:
...
def verify_working_source(
    ...
    *,
    ...
    if actual_tree_sha256 != expected_tree_sha256:
        raise SourceIdentityError(
            "AutoSegRNN source-tree SHA-256 mismatch: "
