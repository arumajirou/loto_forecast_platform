"""Isolated real-provider worker for AutoSegRNN runtime certification."""
...
def _fitted_model_class(model: Any) -> str:
    ...
    if nested is not None:
        return type(nested).__name__
    return type(model).__name__


def _fixed_model_config(request: AutoSegRNNRuntimeRequest) -> dict[str, Any]:
...
def _run_provider(
    ...
    *,
    ...
    alias = f"AutoSegRNN-{request.execution_mode}"
    ...
    forecast.save(path=str(bundle_path), overwrite=True, save_dataset=True)
    ...
    return AutoSegRNNWorkerResponse(
        status="PASS",
        run_label=run_label,
