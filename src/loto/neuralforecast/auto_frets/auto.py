"""NeuralForecast AutoFreTS class factory."""
...
def build_auto_frets_class(
    *,
    ...
    frets_class: type[Any],
    losses: Any,
    random_sampler: type[Any],
    tune: Any,
    module_name: str,
) -> type[Any]:
    class AutoFreTS(base_auto):
        """Ray/Optuna AutoModel wrapper for the local FreTS model."""
        ...
        def __init__(
            ...
            if search_alg is None:
                if backend == "ray":
                    ...
                    raise ValueError(f"unsupported AutoFreTS backend: {backend}")
            super().__init__(
                cls_model=frets_class,
                h=h,
                loss=loss,
    ...
    AutoFreTS.__name__ = "AutoFreTS"
    AutoFreTS.__qualname__ = "AutoFreTS"
    AutoFreTS.__module__ = module_name
    return AutoFreTS
