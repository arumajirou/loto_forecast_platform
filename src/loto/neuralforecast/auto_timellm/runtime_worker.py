"""Isolated real-provider worker for AutoTimeLLM runtime certification."""
...
def synthetic_values(length: int, seed: int) -> list[float]:
    """Produce deterministic bounded values without reading project datasets."""

    return [float(((index * 7) + (seed * 3)) % 37 + 1) for index in range(length)]


def _prediction_matrix(frame: Any, *, alias: str, horizon: int) -> tuple[tuple[float, ...], ...]:
...
def parse_nvidia_smi_output(text: str, provider_pid: int) -> tuple[GPUProcessSampleRecord, ...]:
...
def _capture_gpu_samples(provider_pid: int) -> tuple[GPUProcessSampleRecord, ...]:
    command = [
        "nvidia-smi",
...
def _model_device(model: Any) -> str:
    ...
    if not callable(parameters):
        raise RuntimeError("fitted model does not expose parameters()")
    try:
        parameter = next(parameters())
    except StopIteration as exc:
        raise RuntimeError("fitted model does not expose a parameter device") from exc
    device_type = getattr(getattr(parameter, "device", None), "type", None)
    if device_type == "cuda":
        return "cuda"
    if device_type == "cpu":
        return "cpu"
    raise RuntimeError(f"unsupported fitted-model device: {device_type!r}")
...
def _run_provider(
    ...
    *,
    ...
    from .runtime import get_pinned_timellm_class
    ...
    if installed_version != request.expected_neuralforecast_version:
        raise RuntimeError(
            ...
            f"expected {request.expected_neuralforecast_version}, got {installed_version}"
        )
    ...
    alias = "AutoTimeLLMRuntime"
    model_class = get_pinned_timellm_class()
    accelerator = "gpu" if request.requested_device == "cuda" else "cpu"
    model = model_class(
        h=request.horizon,
        architecture_profile=request.architecture_profile.value,
        llm_identity=request.llm_identity.model_dump(mode="json"),
        max_steps=request.max_steps,
        val_check_steps=request.val_check_steps,
    ...
    maximum_difference = _maximum_difference(pre_reload, post_reload)
    if maximum_difference > request.replay_tolerance:
        raise RuntimeError(
            "save/reload replay exceeded tolerance: "
            f"difference={maximum_difference}, tolerance={request.replay_tolerance}"
        )

    fitted_model = reloaded.models[0]
    effective_device = _model_device(fitted_model)
    ...
    return AutoTimeLLMWorkerResponse(
        status="PASS",
        run_label=run_label,
...
def run_worker(request_path: Path, output_path: Path, run_label: str) -> int:
    provider_pid = os.getpid()
    try:
    ...
    except Exception as exc:  # runtime boundary must retain structured failure evidence
        try:
            requested_device = load_runtime_request(request_path).requested_device
        except Exception:
        ...
        _atomic_write_json(output_path, response.model_dump(mode="json"))
        return 2
    _atomic_write_json(output_path, response.model_dump(mode="json"))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--run-label", choices=("run-a", "run-b"), required=True)
    return parser
