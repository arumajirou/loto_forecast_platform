...
class TemporalPartition(BaseModel):
    ...
    def validate_order(self) -> TemporalPartition:
        ...
        if not ordered:
            raise ValueError(
                "Train, calibration, and evaluation must be ordered and non-overlapping"
            )
        return self
...
class EnsembleConfig(BaseModel):
    ...
    def validate_config(self) -> EnsembleConfig:
        ...
        _validate_protected_args(
            self.predict_args,
            {"n", "series", "num_samples", "predict_likelihood_parameters"},
            "predict",
        )
        return self
...
def build_ensemble_plan(
    ...
    evidence: Mapping[str, BaseModelEvidence],
    *,
    constructor: Callable[..., Any] | None = None,
    fit_method: Callable[..., Any] | None = None,
    predict_method: Callable[..., Any] | None = None,
) -> EnsemblePlan:
    ...
    if config.predict_likelihood_parameters:
        ...
        if len(likelihoods) != 1 or None in likelihoods or len(quantiles) != 1:
            raise P10ContractError(
                "likelihood-parameter ensembling requires identical likelihoods and quantiles"
            )
    ledger: list[ArgumentLedgerEntry] = []
    if constructor is not None:
        _, decisions = classify_arguments(
            constructor,
            config.constructor_args,
            target_name=config.public_name,
            phase="constructor",
        )
        ledger.extend(decisions)
    if fit_method is not None:
        _, decisions = classify_arguments(
            fit_method, config.fit_args, target_name=config.public_name, phase="fit"
        )
        ledger.extend(decisions)
    if predict_method is not None:
        _, decisions = classify_arguments(
            predict_method, config.predict_args, target_name=config.public_name, phase="predict"
        )
        ledger.extend(decisions)
    effective_samples = config.regression_train_num_samples
    return EnsemblePlan(
        public_name=config.public_name,
        base_model_ids=config.base_model_ids,
        output_chunk_shift=next(iter(shifts)),
...
def validate_conformal_base(
    config: ConformalConfig, evidence: Mapping[str, BaseModelEvidence], partition: TemporalPartition
) -> BaseModelEvidence:
    ...
    if config.public_name == "ConformalQRModel":
        ...
        if model.quantiles and (not set(config.quantiles).issubset(model.quantiles)):
            raise P10ContractError(
                "ConformalQRModel requested quantiles are absent from base-model evidence"
            )
