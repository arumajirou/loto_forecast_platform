...
def compute_interval_metrics(actual: np.ndarray | Sequence[Sequence[float]], lower: np.ndarray
    | Sequence[Sequence[float]], upper: np.ndarray | Sequence[Sequence[float]], *,
    lower_quantile: float, upper_quantile: float) -> IntervalMetric:
    ...
    return IntervalMetric(lower_quantile=lower_quantile, upper_quantile=upper_quantile,
        nominal_coverage=nominal, empirical_coverage=empirical, coverage_gap=empirical - nominal,
...
def certify_conformal_quantiles(config: ConformalConfig, partition: TemporalPartition,
    actual: np.ndarray | Sequence[Sequence[float]], quantile_predictions: Mapping[float,
    np.ndarray], *, base_median_prediction: np.ndarray | None=None,
    atol: float=1e-12) -> ConformalCertification:
    ...
    for lower_q, upper_q in zip(expected[:half], reversed(expected[half + 1:])):
        intervals.append(compute_interval_metrics(actual, quantile_predictions[lower_q],
            quantile_predictions[upper_q], lower_quantile=lower_q, upper_quantile=upper_q))
    calibration_indices = tuple(range(partition.calibration_start, partition.calibration_end,
        config.cal_stride))
    ...
    return ConformalCertification(public_name=config.public_name, quantiles=expected,
        non_crossing=True, median_base_parity=median_parity, interval_metrics=tuple(intervals),
