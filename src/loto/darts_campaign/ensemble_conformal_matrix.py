...
def run_p10_matrix(config: P10CampaignConfig, runtime: MatrixRuntime) -> tuple[MatrixResult, ...]:
    tasks = tuple((MatrixTask(public_name=public_name, seed=seed, fold_id=fold_id)
        for public_name in P10_MODEL_IDENTITIES for seed in config.seeds
    ...
    for task in tasks:
        try:
            ...
            results.append(MatrixResult(task=task, status='FAILED',
