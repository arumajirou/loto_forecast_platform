...
def certify_naive_average(base_predictions: Mapping[str, np.ndarray],
    ensemble_prediction: np.ndarray, *, base_model_ids: Sequence[str],
    ...
    return {'base_model_ids': tuple(base_model_ids), 'shape': tuple((int(value)
        for value in observed.shape)),
        'max_abs_delta': float(np.max(np.abs(observed - expected))),
        'prediction_sha256': canonical_sha256(observed.tolist())}

def certify_stacking_evidence(evidence: StackingEvidence, *,
    expected_base_model_ids: Sequence[str], expected_seeds: Sequence[int],
    expected_fold_ids: Sequence[int], partition: TemporalPartition) -> dict[str, Any]:
    ...
    training_keys = {(item.seed, item.fold_id, item.target_index, item.position)
        for item in evidence.training_records}
    evaluation_keys = {(item.seed, item.fold_id, item.target_index, item.position)
        for item in evidence.evaluation_records}
    if any((not partition.train_start <= item.origin < partition.calibration_end
        or item.target_index >= partition.evaluation_start for item in evidence.training_records)):
    ...
    if any((item.origin < partition.evaluation_start
        or item.target_index >= partition.evaluation_end for item in evidence.evaluation_records)):
    ...
    return {'training_record_count': len(evidence.training_records),
