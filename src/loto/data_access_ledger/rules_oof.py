...
def validate_oof(ledger: DataAccessLedger) -> list[ValidationFinding]:
    ...
    for key, predict_events in predicts.items():
        model_fits = [
            item for item in fits.get(key, []) if item.operation is AccessOperation.FIT_MODEL
        ]
            ...
            if not prior:
                findings.append(
                    finding(
                        ...
                        message=(
                            "OOF prediction has no earlier model fit for the same fold and seed"
                        ),
                    )
                )
                continue
            fit = prior[-1]
            train_slices = [item for item in fit.input_slices if item.fold_role is FoldRole.TRAIN]
            validation_slices = [
                item for item in predict.input_slices if item.fold_role is FoldRole.VALIDATION
            ]
            if (
                not train_slices
                or not validation_slices
                or max(item.observed_time_end for item in train_slices)
                ...
                findings.append(
                    finding(
                        ...
                        message=(
                            "OOF chronology requires train_observed_end < validation_observed_start"
                        ),
                    )
                )
    ...
    for (fold_id, seed), fit_events in fits.items():
        model_fits = [item for item in fit_events if item.operation is AccessOperation.FIT_MODEL]
        if model_fits:
            train = [
                item for item in model_fits[-1].input_slices if item.fold_role is FoldRole.TRAIN
            ]
    ...
    if ledger.expected_seeds:
        ...
        for fold_id in sorted(folds):
            observed = {
                event.seed
                for event in oof_events
                if event.fold_id == fold_id and event.operation is AccessOperation.PREDICT
            }
