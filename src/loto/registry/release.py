...
def create_release_bundle(
    release_id: str, artifacts: list[str | Path], output_path: str | Path
) -> dict:
    ...
    bundle = {
        ...
        "artifacts": [
            {"path": str(p), "sha256": _sha(p), "size_bytes": p.stat().st_size} for p in paths
        ],
    }
...
def verify_release_bundle(bundle: dict | str | Path) -> bool:
    ...
    if hashlib.sha256(canonical).hexdigest() != bundle.get("bundle_sha256"):
        return False
    return all(
        Path(a["path"]).exists() and _sha(Path(a["path"])) == a["sha256"]
