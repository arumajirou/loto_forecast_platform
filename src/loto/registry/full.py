...
class PlatformRegistry:
    ...
    def audit(self, actor: str, action: str, object_type: str, object_id: str,
        ...
        with self.db.connect() as con:
            con.cursor().execute(
                ...
                (self._now(), actor, action, object_type, object_id, reason, json.dumps(payload or {}, ensure_ascii=False, default=str)),
            )
    ...
    def update_run(self, run_id: str, *, status: str, current_stage: str | None = None,
                   error: dict | None = None, release_id: str | None = None) -> None:
        p = self.db.placeholder
        with self.db.connect() as con:
            con.cursor().execute(
                f"UPDATE runs SET status={p},current_stage={p},updated_at={p},error_json={p},release_id=COALESCE({p},release_id) WHERE run_id={p}",
                (status, current_stage, self._now(), json.dumps(error) if error else None, release_id, run_id),
            )

    def record_task(self, run_id: str, stage: str, status: str, *, attempt: int = 1,
                    input_hash: str = "", output_uri: str | None = None,
        ...
        with self.db.connect() as con:
            con.cursor().execute(sql, (run_id, stage, status, attempt, started, finished, input_hash,
                                       output_uri, json.dumps(error) if error else None))

    def completed_stages(self, run_id: str) -> set[str]:
        p = self.db.placeholder
        with self.db.connect() as con:
            rows = con.cursor().execute(
                f"SELECT DISTINCT stage FROM tasks WHERE run_id={p} AND status='SUCCEEDED'", (run_id,)
            ).fetchall()
        return {str(r[0]) for r in rows}

    def register_model(self, model_id: str, model_type: str, artifact_uri: str,
                       metrics: dict, metadata: dict, status: str = "CANDIDATE") -> None:
        p = self.db.placeholder
        with self.db.connect() as con:
            con.cursor().execute(
                f"INSERT INTO models VALUES({','.join([p]*7)}) ON CONFLICT(model_id) DO NOTHING",
                (model_id, model_type, status, artifact_uri, json.dumps(metrics), json.dumps(metadata), self._now()),
            )

    def register_forecast(self, forecast_id: str, run_id: str, draw_id: str,
                          sealed: dict, verified: bool, status: str = "SEALED") -> None:
        p = self.db.placeholder
        with self.db.connect() as con:
            con.cursor().execute(
                ...
                (forecast_id, run_id, draw_id, status, json.dumps(sealed, ensure_ascii=False), int(verified), self._now()),
            )
    ...
    def request_approval(self, object_type: str, object_id: str, action: str,
        ...
        with self.db.connect() as con:
            con.cursor().execute(
                ...
                (object_type, object_id, action, requested_by, None, "PENDING", reason, self._now(), None),
            )

    def decide_approval(self, object_type: str, object_id: str, action: str,
        ...
        with self.db.connect() as con:
            ...
            con.cursor().execute(
                ...
                (approved_by, "APPROVED" if approved else "REJECTED", self._now(), object_type, object_id, action),
            )

    def list_rows(self, table: str, limit: int = 100) -> list[dict]:
        if table not in {"runs", "tasks", "models", "forecasts", "releases", "approvals", "audit_log"}:
            raise ValueError("unsupported table")
        with self.db.connect() as con:
            rows = con.cursor().execute(f"SELECT * FROM {table} ORDER BY rowid DESC LIMIT {int(limit)}" if self.db.kind == "sqlite" else f"SELECT * FROM {table} LIMIT {int(limit)}").fetchall()
        return [dict(r) if hasattr(r, "keys") else {str(i): v for i, v in enumerate(r)} for r in rows]
