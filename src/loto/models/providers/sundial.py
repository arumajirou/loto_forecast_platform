...
class SundialProvider(FoundationProvider):
    ...
    def validate_environment(self) -> dict[str, Any]:
        if not SUNDIAL_ENV.exists():
            raise FoundationProviderError("DEPENDENCY_MISSING", f"missing Sundial environment: {SUNDIAL_ENV}")
        if not (SUNDIAL_ENV / "uv.lock").exists():
            raise FoundationProviderError("DEPENDENCY_MISSING", f"missing Sundial lockfile: {SUNDIAL_ENV / 'uv.lock'}")
        if not SUNDIAL_RUNNER.exists():
            raise FoundationProviderError("PROVIDER_NOT_IMPLEMENTED", f"missing Sundial runner: {SUNDIAL_RUNNER}")
        return {
    ...
    def _run_provider(self, history: pd.DataFrame) -> dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory(prefix="loto-sundial-") as tmp:
            ...
            try:
                ...
                raise FoundationProviderError("PREDICT_FAILED", f"Sundial provider returned invalid JSON: {exc}") from exc
