...
class MoiraiProvider(FoundationProvider):
    ...
    def validate_environment(self) -> dict[str, Any]:
        if not MOIRAI_ENV.exists():
            raise FoundationProviderError("DEPENDENCY_MISSING", f"missing Moirai environment: {MOIRAI_ENV}")
        if not (MOIRAI_ENV / "uv.lock").exists():
            raise FoundationProviderError("DEPENDENCY_MISSING", f"missing Moirai lockfile: {MOIRAI_ENV / 'uv.lock'}")
        if not MOIRAI_RUNNER.exists():
            raise FoundationProviderError("PROVIDER_NOT_IMPLEMENTED", f"missing Moirai runner: {MOIRAI_RUNNER}")
        return {
    ...
    def _run_provider(self, history: pd.DataFrame) -> dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory(prefix="loto-moirai-") as tmp:
            ...
            try:
                ...
                raise FoundationProviderError("PREDICT_FAILED", f"Moirai provider returned invalid JSON: {exc}") from exc
