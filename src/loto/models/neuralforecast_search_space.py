...
def _finish(*, backend: str, source: str, completeness: SearchSpaceCompleteness,
            dimensions: list[dict[str, Any]], model_name: str | None,
            conditional: bool | None = False, errors: tuple[str, ...] = ()) -> SearchSpaceProfile:
    ...
    return SearchSpaceProfile(
        ...
        cmaes_eligible=(
            backend == "optuna" and complete and tunable > 0
            and categorical == 0 and not conditional
        ),
        grid_eligible=(
            complete and combinations is not None
            and 1 < combinations <= 256 and not conditional
        ),
        probe_errors=errors,
    )
...
def profile_fixed_config(config: dict[str, Any], *, backend: str,
                         model_name: str | None = None) -> SearchSpaceProfile:
    return _finish(
        backend=backend,
        source="fixed_config",
...
def profile_ray_config(config: dict[str, Any], *,
                       model_name: str | None = None) -> SearchSpaceProfile:
    ...
    for name, value in config.items():
        ...
        if categories is not None:
            ...
            dimensions.append({
                ...
                "choices": values, "cardinality": len(values),
            })
            continue
...
class _Trial:
    ...
    def suggest_categorical(self, name: str, choices: list[Any]) -> Any:
        self.dimensions[name] = {
            "name": name, "kind": "categorical",
            "choices": choices, "cardinality": len(choices),
        }
        return self._pick(choices)

    def suggest_int(self, name: str, low: int, high: int, *,
                    step: int = 1, log: bool = False) -> int:
        self.dimensions[name] = {
            "name": name, "kind": "integer", "lower": low, "upper": high,
            "step": step, "log": log, "cardinality": (high - low) // step + 1,
        }
        return int(self._pick(list(range(low, high + 1, step))))

    def suggest_float(self, name: str, low: float, high: float, *,
                      step: float | None = None, log: bool = False) -> float:
        card = int(math.floor((high - low) / step)) + 1 if step else None
        self.dimensions[name] = {
            "name": name, "kind": "float", "lower": low, "upper": high,
            "step": step, "log": log, "cardinality": card,
        }
        if self.mode == "low":
            return low
        if self.mode == "high":
...
def profile_optuna_config(config: Callable[[Any], dict[str, Any]], *,
                          model_name: str | None = None) -> SearchSpaceProfile:
    ...
    if not probes:
        return _finish(
            backend="optuna", source="optuna_recording_trial",
            completeness=SearchSpaceCompleteness.FAILED, dimensions=[],
            model_name=model_name, conditional=None, errors=tuple(errors),
        )
    ...
    return _finish(
        backend="optuna", source="optuna_recording_trial",
        completeness=SearchSpaceCompleteness.PARTIAL, dimensions=dimensions,
        model_name=model_name, conditional=conditional, errors=tuple(errors),
    )


def unavailable_search_space_profile(*, backend: str, model_name: str | None,
                                     reason: str) -> SearchSpaceProfile:
    return _finish(
        backend=backend, source="unavailable",
        completeness=SearchSpaceCompleteness.UNAVAILABLE,
        dimensions=[], model_name=model_name, conditional=None, errors=(reason,),
    )


def profile_runtime_model_config(model: Any, *, backend: str,
                                 model_name: str | None,
                                 fallback: SearchSpaceProfile | None = None) -> SearchSpaceProfile:
...
def write_search_space_profile(directory: str | Path,
                               profile: SearchSpaceProfile) -> dict[str, str]:
    root = Path(directory)
    ...
    files = [
        ...
        (root / "SEARCH_SPACE_PROFILE.sha256",
         f"{digest}  SEARCH_SPACE_PROFILE.json\n".encode()),
    ]
