...
def decode_hybrid(candidate_scores: np.ndarray, position_scores: np.ndarray, *, top_k: int = 20,
                  candidate_weight: float = 1.0, position_weight: float = 1.0,
                  cooccurrence: np.ndarray | None = None, cooccurrence_weight: float = 0.0) -> list[DecodedCombination]:
    ...
    for n in range(1, 38):
        if 37 - n < 6:
            continue
        score = candidate_weight * candidate_scores[n - 1] + position_weight * position_scores[0, n - 1]
    ...
    for pos in range(1, 7):
        ...
        for n in range(pos + 1, 38):
            ...
            add = candidate_weight * candidate_scores[n - 1] + position_weight * position_scores[pos, n - 1]
    ...
    for score, path in all_paths:
        if cooccurrence is not None and cooccurrence_weight:
            pair_score = sum(float(cooccurrence[a - 1, b - 1]) for i, a in enumerate(path) for b in path[i + 1:])
            score += cooccurrence_weight * pair_score
        rescored.append((score, path))
    rescored.sort(key=lambda x: (-x[0], x[1]))
    return [DecodedCombination(numbers=list(path), score=float(score)) for score, path in rescored[:top_k]]
