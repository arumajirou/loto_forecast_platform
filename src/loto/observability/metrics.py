...
try:
    ...
    RESEARCH_TRIAL_TOTAL = Counter("loto_research_trial_total", "Research trials", ["model_id", "status"])
    RESEARCH_TRIAL_DURATION = Histogram("loto_research_trial_duration_seconds", "Research trial duration", ["model_id"])
    RESEARCH_METRIC = Gauge("loto_research_metric", "Latest research metric", ["model_id", "metric"])
    DATA_GAME_TOTAL = Counter("loto_data_game_total", "Data pipeline games", ["game", "status"])
    NOTIFICATION_TOTAL = Counter("loto_notification_total", "Notification attempts", ["channel", "status"])
...
def observe_trial(model_id: str, status: str, duration: float, metrics: dict[str, float] | None = None) -> None:
