...
SEED = 1
OBJECT_REPR_RE = re.compile(r"^<(?P<class>[A-Za-z_][A-Za-z0-9_.]+) object at 0x[0-9A-Fa-f]+>$")
...
def normalize_serialized(value: Any) -> Any:
    if isinstance(value, dict):
        return {str(k): normalize_serialized(v) for k, v in sorted(value.items(), key=lambda kv: str(kv[0]))}
...
def typed_tree(value: Any, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        return {**base, "items": [{"key": typed_tree(k, depth + 1), "value": typed_tree(v, depth + 1)} for k, v in value.items()]}
    if isinstance(value, (list, tuple)):
        return {**base, "items": [typed_tree(v, depth + 1) for v in value]}
    if callable(value):
        return {**base, "callable": True, "module": getattr(value, "__module__", None), "qualname": getattr(value, "__qualname__", getattr(value, "__name__", None)), "repr": repr(value)}
    state: dict[str, Any] = {}
    try:
...
def exact_diff(left: Any, right: Any, path: str = "$") -> list[dict[str, Any]]:
    ...
    if isinstance(left, list) and isinstance(right, list):
        if len(left) != len(right):
            out.append({"path": path, "kind": "LENGTH", "left": len(left), "right": len(right)})
        for idx, (lv, rv) in enumerate(zip(left, right)):
            out.extend(exact_diff(lv, rv, f"{path}[{idx}]"))
        return out
    if type(left) is not type(right) or left != right:
        out.append({"path": path, "kind": "TYPE" if type(left) is not type(right) else "VALUE", "left": left, "right": right, "left_type": f"{type(left).__module__}.{type(left).__qualname__}", "right_type": f"{type(right).__module__}.{type(right).__qualname__}"})
    return out
...
def recursive_diff(left: Any, right: Any, path: str = "$") -> list[dict[str, Any]]:
    ...
    if isinstance(left, (int, float)) and isinstance(right, (int, float)) and not isinstance(left, bool) and not isinstance(right, bool):
        same = bool(np.isclose(float(left), float(right), rtol=1e-12, atol=1e-12, equal_nan=True))
    else:
        same = left == right
...
def compare_trials(expected_path: Path, replay: pd.DataFrame, out_csv: Path) -> dict[str, Any]:
    ...
    numbers_same = np.array_equal(expected["number"].to_numpy(int), replay["number"].to_numpy(int))
    states_same = np.array_equal(expected["state"].astype(str).to_numpy(), replay["state"].astype(str).to_numpy())
    ev = pd.to_numeric(expected["value"], errors="coerce").to_numpy(float)
    rv = pd.to_numeric(replay["value"], errors="coerce").to_numpy(float)
    objectives_same = len(ev) == len(rv) and bool(np.allclose(ev, rv, rtol=0.0, atol=1e-10, equal_nan=True))
    ...
    for col in params:
        ...
        for idx in range(min(len(expected), len(replay))):
            ...
            if pd.isna(lv) or pd.isna(rvv):
                ...
                try:
                    same = bool(np.isclose(float(lv), float(rvv), rtol=1e-12, atol=1e-12, equal_nan=True))
                except (TypeError, ValueError):
                    same = str(lv) == str(rvv)
            if not same:
                params_same = False
                rows.append({"trial": int(expected.iloc[idx]["number"]), "parameter": col, "expected": None if pd.isna(lv) else lv, "replay": None if pd.isna(rvv) else rvv})
    with out_csv.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["trial", "parameter", "expected", "replay", "kind"], extrasaction="ignore")
        writer.writeheader()
        writer.writerows(rows)
    return {"trial_count_expected": len(expected), "trial_count_replay": len(replay), "numbers_same": bool(numbers_same), "states_same": bool(states_same), "objectives_same": objectives_same, "params_same": bool(params_same), "param_diff_rows": len(rows)}
...
def environment(repo: Path, runner: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "packages": {name: package_version(name) for name in ["mlforecast", "optuna", "catboost", "lightgbm", "scikit-learn", "numpy", "pandas"]},
...
def environment_diff(old: dict[str, Any] | None, current: dict[str, Any]) -> dict[str, Any]:
    if old is None:
        return {"historical_available": False, "differences": [], "version_drift_detected": False}
    diffs = []
    pairs = {
    ...
    for key, (left, right) in pairs.items():
        if left is not None and right is not None:
            same = str(left).split()[0] == str(right).split()[0] if key == "python" else str(left) == str(right)
            if not same:
                diffs.append({"field": key, "frozen": left, "current": right})
    return {"historical_available": True, "differences": diffs, "version_drift_detected": bool(diffs)}


def progress(out: Path, phase: str, percent: int, completed: int, total: int, status: str = "RUNNING") -> None:
    write_json(out / "DIAGNOSIS_PROGRESS.json", {"phase": phase, "progress_percent": percent, "current_seed": SEED, "completed": completed, "total": total, "status": status, "updated_at_unix": time.time()})


def main() -> int:
    parser = argparse.ArgumentParser(description="READ-ONLY Phase7 semantic config diagnosis; no Holdout actual access")
    ...
    present, _ = git_text(repo, "cat-file", "-e", f"{EXPECTED_EXPERIMENT_GIT_COMMIT}^{{commit}}")
    if present != 0:
        raise RuntimeError("frozen experiment commit is not present in this checkout")
    ancestor, _ = git_text(repo, "merge-base", "--is-ancestor", EXPECTED_EXPERIMENT_GIT_COMMIT, "HEAD")
    if ancestor != 0:
    ...
    if holdout_draws != 0 or actuals_accessed != 0:
        raise RuntimeError(f"Holdout integrity gate failed: draws={holdout_draws}, actuals={actuals_accessed}")
    lock_dir = p7 / "artifacts" / "prediction_locks"
    lock_files = sorted(p.name for p in lock_dir.glob("*") if p.is_file()) if lock_dir.exists() else []
    if lock_files:
    ...
    component = next((c for c in freeze.get("components", []) if c.get("model") == "AutoCatboost" and int(c.get("seed")) == SEED), None)
    if component is None:
        raise RuntimeError("seed=1 AutoCatboost component missing")
    if str(component.get("objective")) != "raw_hit_at_1" or int(component.get("num_samples", -1)) != 20 or not np.isclose(float(component.get("weight", float("nan"))), 0.25, atol=1e-12, rtol=0.0):
    ...
    if sha256_file(frozen_config_path) != str(component["frozen_config_sha256"]) or sha256_file(frozen_trials_path) != str(component["frozen_trials_sha256"]):
    ...
    frozen_semantic = str(frozen_doc.get("config_sha256") or component.get("semantic_config_sha256"))
    ...
    auto = runner.AutoMLForecast(models={"AutoCatboost": runner.AutoCatboost()}, freq=1, season_length=1, fit_config=fit_config, num_threads=1, reuse_cv_splits=True)
    sampler = runner.optuna.samplers.TPESampler(seed=SEED)
    progress(out, "REPLAY_HPO", 35, 0, int(runner.NUM_SAMPLES))
    auto.fit(df=hpo_panel, n_windows=int(runner.INNER_WINDOWS), h=1, num_samples=int(runner.NUM_SAMPLES), step_size=1, input_size=None, refit=False, loss=runner.raw_hit_loss, id_col="unique_id", time_col="ds", target_col="y", study_kwargs={"sampler": sampler}, optimize_kwargs={"n_jobs": 1})
    ...
    trial_cmp = compare_trials(frozen_trials_path, replay_trials, out / "TRIAL_PARAM_DIFF.csv")
    ...
    frozen_canonical, replay_canonical = normalize_serialized(frozen_config), normalize_serialized(replay_serialized)
    ...
    write_json(out / "RAW_REPLAY_CONFIG.json", {"best_trial": int(study.best_trial.number), "best_inner_loss": float(study.best_value), "config": replay_serialized, "config_sha256": replay_semantic})
    ...
    write_json(out / "SEMANTIC_CONFIG_DIFF.json", {"raw_diff_count": len(raw_diffs), "raw_differences": raw_diffs, "tolerant_raw_diff_count": len(tolerant_diffs), "tolerant_raw_differences": tolerant_diffs, "canonical_diff_count": len(canonical_diffs), "canonical_differences": canonical_diffs, "all_raw_differences_representation_only": repr_only})

    best_trial_same = int(frozen_doc["best_trial"]) == int(study.best_trial.number)
    best_objective_same = bool(np.isclose(float(frozen_doc["best_inner_loss"]), float(study.best_value), rtol=0.0, atol=1e-10, equal_nan=True))
    trial_params_same = bool(trial_cmp["numbers_same"] and trial_cmp["states_same"] and trial_cmp["objectives_same"] and trial_cmp["params_same"])
    ...
    if repr_only and semantic_values_same and best_trial_same and best_objective_same and trial_params_same and not version_drift and bridge_result["canonical_semantic_match"] is True:
        ...
        next_action = "Reconstruct the original Phase6B runtime and repeat this development-only replay."
    elif true_drift:
        classification, status, safe_change = "TRUE_CONFIG_DRIFT", "BLOCKED", False
        reason = "Best trial/trial sequence or canonical semantic values differ beyond representation-only normalization."
        next_action = "Keep Holdout sealed and investigate the development-only reproducibility contract."
    else:
    ...
    diagnosis = {
        ...
        "diagnosis_note": "No canonical/Holdout dataset path is accepted by this program; Phase7 and Candidate Freeze are read-only.",
    }
    write_json(out / "DIAGNOSIS.json", diagnosis)

    md = ["# Phase 7 Semantic Config Diagnosis", "", f"- Status: **{status}**", f"- Classification: **{classification}**", f"- Holdout draws accessed: **{holdout_draws}**", f"- Actuals accessed: **{actuals_accessed}**", "", "## Reason", "", reason, "", "## Raw differences", ""]
    md.extend([f"- `{d['path']}` {d['kind']}: `{d.get('left')}` -> `{d.get('right')}`" for d in raw_diffs] or ["- None"])
    md += ["", "## Canonical differences", ""]
    md.extend([f"- `{d['path']}` {d['kind']}: `{d.get('left')}` -> `{d.get('right')}`" for d in canonical_diffs] or ["- None"])
    md += ["", "## Next action", "", next_action, ""]
    (out / "SEMANTIC_CONFIG_DIFF.md").write_text("\n".join(md), encoding="utf-8")
    (out / "README.md").write_text("# Phase7 semantic diagnosis artifacts\n\nREAD-ONLY development-only forensic replay. Holdout and Candidate Freeze remain untouched.\n", encoding="utf-8")
    progress(out, "COMPLETE", 100, int(runner.NUM_SAMPLES), int(runner.NUM_SAMPLES), status)

    manifest = []
    for path in sorted(out.iterdir()):
        if path.is_file() and path.name not in {"ARTIFACT_MANIFEST.json", "SHA256SUMS", "launcher.log"}:
            manifest.append({"file": path.name, "bytes": path.stat().st_size, "sha256": sha256_file(path)})
    write_json(out / "ARTIFACT_MANIFEST.json", {"holdout_accessed": False, "files": manifest})
    lines = [f"{sha256_file(path)}  {path.name}" for path in sorted(out.iterdir()) if path.is_file() and path.name not in {"SHA256SUMS", "launcher.log"}]
